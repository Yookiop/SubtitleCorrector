...
```text
dubbelklik tools\test-lang-utils.html     # 61 tests van de pure logica, geen Node nodig
...
| De bovenste regel wordt afgekapt (de letters missen bovenaan) | Herlaad de extensie (`brave://extensions` → ⟳) zodat de nieuwste `page-agent.js` actief is. Sinds de fix van 2026-09-18 meet het rollende venster de **echte** afstand tussen de regels (`L.captionLineMetrics()` → `L.captionWindowShiftLines()`, vensterhoogte via `L.captionBoxHeightEmForAdvance()`) en zet onze CSS de `line-height`/`font-size` van de wrapper en de woordblokjes met `!important` vast. Geeft een pagina (of een andere extensie) de woordblokjes tóch een andere regelafstand, dan blijft de bovenste regel nu binnen het venster in plaats van erboven. Controle: `tools\check-caption-window.html`. |
