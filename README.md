...
* de blokjes van regels die **buiten het venster** vallen schilderen daar geen zwart: hun verticale padding valt bij een inline-element buiten de regelbox, en zonder ingrijpen stak de regel die net boven het venster staat met zijn onderpadding onder de bovenrand uit — en omdat zo'n weggeschoven regel bijna altijd verder doorloopt dan de bovenste zichtbare regel (die breekt af op het woord dat niet meer paste), zag je een strook zwart naast het laatste woord van die regel (bugmelding 2026-09-20; zie rij 28 van de samenvatting in `SUBTITLE_CORRECTOR_PROMPT.md`). `L.captionVisibleGroupRange()` bepaalt welke regels in het venster vallen en alleen die krijgen aan de kant van het venster geen verticale padding; de fractie van een px die de regel erboven dan nog uitsteekt (zijn *inhoud*) schuift `L.captionVerticalLayout()` weg via `pokePx`. Dat is geen analytische fractie maar een **keep-out**: de verschuiving gaat minimaal naar `padding − halfLeading + 1,5 px` en de hoogte gaat met het dubbele mee omlaag, zodat de inkt boven en onder even ver van de rand blijft (de gemeten inktmarge in een woordblokje wijkt namelijk tot ~1 px af van de theoretische `halfLeading`). `tools/check-caption-window.html` meet dit per stap na (horizontaal én verticaal: 0 px met de fix, 83 px / 11 px met `?nofix=1`);
...
```text
dubbelklik tools\test-lang-utils.html     # 64 tests van de pure logica, geen Node nodig
